...
use qdrant_client::{
    ...
    qdrant::{
        ...
        Distance, PointStruct, ScrollPoints, SearchPoints, VectorParams,
        VectorsConfig, WithPayloadSelector, FieldCondition, Filter, Condition, Match,
        Value, PointsSelector, UpsertPoints, DeletePoints, DeleteCollectionBuilder,
    },
};
use rig::{
...
impl QdrantRagEngine {
    pub async fn new(
        ...
        // 获取嵌入模型的向量维度
        let vector_size = embed_model.embed(&["test".to_string()]).await?
            .first()
            .map(|v| v.len())
            .unwrap_or(1536);
    ...
    async fn create_collection(&self) -> Result<()> {
        ...
        self.client
            ...
            .map_err(|e| KbError::VectorStore {
                ...
                message: format!("Failed to create collection {}: {}", self.collection_name, e),
            })?;
    ...
    async fn vector_search_with_request(
        ...
        self.vector_search(query_embedding, req.samples() as usize, filters).await
    }
            ...
            for (k, v) in payload {
                if let Some(kind) = v.kind {
                    let json_value = match kind {
                        qdrant_client::qdrant::value::Kind::StringValue(s) => serde_json::Value::String(s),
                        qdrant_client::qdrant::value::Kind::IntegerValue(i) => serde_json::Value::Number(serde_json::Number::from(i)),
                        ...
                        qdrant_client::qdrant::value::Kind::BoolValue(b) => serde_json::Value::Bool(b),
                        qdrant_client::qdrant::value::Kind::ListValue(list) => {
                            let arr: Vec<serde_json::Value> = list.values.into_iter().map(|item| {
    ...
    pub async fn delete_collection(&self) -> Result<()> {
        let delete_request = DeleteCollectionBuilder::new(&self.collection_name)
            .build();
        ...
        self.client
            ...
            .map_err(|e| KbError::VectorStore {
                ...
                message: format!("Failed to delete collection {}: {}", self.collection_name, e),
            })?;
    ...
    pub async fn remove_document(&self, document_id: &str) -> Result<usize> {
        ...
        match self.client.delete_points(delete_request).await {
            Ok(response) => {
                ...
                info!("Deleted {} points for document {}", deleted_count, document_id);
...
impl RagEngine for QdrantRagEngine {
    ...
    async fn add_document_text_with_meta(
        ...
        let chunks_count = chunks_text.len();
        ...
        // 创建 KnowledgeChunk 结构体
        let mut knowledge_chunks = Vec::new();
        for (i, chunk_text) in chunks_text.into_iter().enumerate() {
            ...
            let chunk = KnowledgeChunk {
                ...
                created_at,
                custom_fields: meta.as_ref().and_then(|m| m.custom_fields.clone()),
            };
        ...
        let embeddings = self
            .base
            .embed_model
            .embed(&knowledge_chunks.iter().map(|c| c.text.clone()).collect::<Vec<_>>())
            .await
            .map_err(|e| KbError::EmbeddingService {
                provider: "qdrant".to_string(),
                message: e.to_string(),
                ...
                let qdrant_value = match v {
                    ...
                    serde_json::Value::Array(arr) => {
                        let list_value = qdrant_client::qdrant::ListValue {
                            values: arr.iter().map(|item| Value {
